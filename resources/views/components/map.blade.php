<div class="container-fluid p-0 position-relative {{ ($show_map) ? 'vh-38' : '' }}" id="map-container">
    <div class="position-absolute bg-light border-bottom vh-38 w-100 {{ ($show_map) ? '' : 'hide' }}" id="map" data-center="{{ json_encode($center) }}" data-zoom="{{ $zoom }}" data-active_id={{ $active_id ?? null }}></div>
