{{-- upload_multiple field customized to work with media-library media --}}
    ...
    <div class="well well-sm existing-file">
        @foreach($values as $key => $media)
    		<div class="file-preview">
                {{ $media->img()->attributes(['width' => '80px', 'class' => 'm-2']) }}
                <span>{{ $media->file_name }}</span>
		    	<a href="#" class="btn btn-light btn-sm float-right file-clear-button" title="Clear file" data-media-id="{{ $media->id }}"><i class="la la-remove"></i></a>
        ...
        <script>
        	function bpFieldInitUploadMultipleElement(element) {
		        ...
		        clearFileButton.click(function(e) {
		        	...
		        	$("<input type='hidden' name='clear_"+fieldName+"[]' value='"+$(this).data('media-id')+"'>").insertAfter(fileInput);
		        });
